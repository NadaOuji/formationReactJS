...
return <form data-testid="MemeForm">
  ...
  <label htmlFor="meme-name">Nom du meme</label><br/>
   ...
   <div className={styles.textEditor} >
     ...
     <div className={styles.inlineContanier}>
       <label>
         <input type="number"value={state.text.x} onChange={(evt) => {
       setstate({...state,text:{...state.text,x:evt.target.value}});
     }} /> :X
       </label> / <label> Y :   <input type="number" value={state.text.y} onChange={(evt) => {
    ...
    <label>
         <input type="checkbox" checked={state.text.underline} onChange={(evt) => {
           setstate({...state,text:{...state.text,underline: evt.target.checked}} );
         }} /> : underline
       </label> / <label> bold: <input type="checkbox" checked={state.text.bold} onChange={(evt) => {
